...
import { Plus, X, Wrench, Search, ArrowLeft, Tag } from 'lucide-react'
import Link from 'next/link'

interface AccType { id: string; name: string }
interface Accessory { id: string; name: string; price: number; type_id: string|null; accessory_types?: {name:string}|null }
...
export default function AccessoriesPage() {
  ...
  const [fTypeId, setFTypeId] = useState(''); const [saving, setSaving] = useState(false)
  ...
  const openAdd = () => { setEditing(null);setFName('');setFPrice('');setFTypeId('');setPanelOpen(true) }
  const openEdit = (a: Accessory) => { setEditing(a);setFName(a.name);setFPrice(String(a.price));setFTypeId(a.type_id||'');setPanelOpen(true) }
  const close = () => { setPanelOpen(false);setEditing(null) }

  const save = async () => {
    if(!fName.trim()) return; setSaving(true)
    const p = { name:fName, price:parseFloat(fPrice)||0, type_id:fTypeId||null, tenant_id:profile?.tenant_id }
